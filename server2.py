...
def threaded_client(connection, client_id):
    connection.send(str.encode(
        'Functionalities : alloc resource_ name resource_requirement and dealloc resource_name resources_to_be_freed\nAvailable resources :'+str(current_resources)))
    while True:
        ...
        if from_client[0] == 'alloc':
            ...
            if len(from_client) != 3:
                ...
                if client_id in allocated_resources:
                    ...
                    reply = "Client " + \
                        str(client_id)+" is already allocated " + \
                        str(already_allocated)
                    print(reply)
                    connection.send(str.encode(reply))
                else:
                    ...
                    if resource_requirement <= current_resources[resource_name]:
                        current_resources[resource_name] = current_resources[resource_name] - \
                            resource_requirement
                        reply = 'allocated '+resource_name+' ' + \
                            str(resource_requirement) + \
                            "\nAvailable resources :"+str(current_resources)
                        ...
                        allocated_resources[client_id] = (
                            resource_name, resource_requirement)
                        print('For client ', client_id, ':', reply)
                        connection.send(str.encode(reply))
                    # resources are not available for client requirement
                    else:
                        reply = 'Not enough resources available : ' + \
                        ...
                        client_reply_for_res = client_reply_for_res.decode(
                            'utf-8')
                        ...
                        if client_reply_for_res == "get":
                            # add client to allocated_resources with its allotted resoure list to prevent the same client for requesting multiple resources
                            allocated_resources[client_id] = (
                                resource_name, current_resources[resource_name])
                            current_resources[resource_name] = 0
                            reply = 'allocated ' + \
                                str(allocated_resources[client_id]) + \
                                '\nAvailable resources :' + \
                                str(current_resources)
                            print('For client ', client_id, ':', reply)
                            connection.send(str.encode(reply))
                        ...
                        if client_reply_for_res == "abort":
                            reply = 'request aborted' + \
                                "\nAvailable resources :" + \
                                    str(current_resources)
                            print(
                                'request for resources aborted by client ', client_id)
                            connection.send(str.encode(reply))
                        # client waits for resource to get freed
                        if client_reply_for_res == "wait":
                            print("Client ", client_id, ' waiting for: ',
                                  resource_name, resource_requirement)
                            # add client to the waiting queue
                            client_queue.append(
                                (client_id, resource_name, resource_requirement))
                            while True:
            ...
            if len(from_client) != 3:
                ...
                if client_id not in allocated_resources:
                    ...
                    reply = "Error: "+resource_name + \
                        " not was allotted to client\nAvailable resources :" + \
                            str(current_resources)
                    connection.send(str.encode(reply))
                else:
                    ...
                    if resource_name != actually_alloted_resources[0]:
                        ...
                        print(
                            "Client ", client_id, " is trying to return resources more or less than allocated")
