...
files = {'file1': 'Contents of file1.',
         'file2': 'Contents of file2.', 'file3': 'Contents of file3.'}
clients = {}
...
def threaded_client(connection, client_id):
    connection.send(str.encode(
        'Functionalities : read "filename" | write "filename" | StopRead "filename" | StopWrite "filename"'))
    while True:
                ...
                if client_id in clients and clients[client_id] == filename:
                    ...
                    if filename in files:
                        ...
                        # check if file is being read already
                            ...
                            print("Client ", client_id,
                                  ' has been granted a Read access')
                    ...
                    # file not present in the server. send error
                        ...
                        print("Client ", client_id,
                              ' provided invalid filename to Read')
        ...
        # stop reading
            ...
            if len(from_client) != 2:
                print("Client ", client_id,
                      ' did not provide filename to StopRead')
                reply = "Error : Invalid command. Please enter filename"
                connection.send(str.encode(reply))
            else:
                ...
                if client_id in clients and clients[client_id] == filename:
                    ...
                    if filename in files:
                        ...
                        if filename not in currently_reading:
                            print("Client ", client_id,
                                  ' provided invalid filename to StopRead')
                            ...
                            print('Reading access of Client',
                                  client_id, ' revoked')
                    ...
                    print("Client ", client_id,
                          ' did not have read access to the requested StopRead file')
        ...
        # write to file
            ...
            if len(from_client) != 2:
                ...
                # file not present in the server storage
                    ...
                    print("Client ", client_id,
                          ' entered invalid filename for write access')
                    reply = "File not found! Enter valid filename"
                    connection.send(str.encode(reply))
            ...
            if len(from_client) != 2:
                print("Client ", client_id,
                      ' did not enter filename for StopWrite access')
                reply = "Error : Invalid command. Please enter filename"
                connection.send(str.encode(reply))
            else:
                # check if file is present in the server storage
                if filename in files:
                    ...
                    # remove writing access from the client
                        ...
                        print('Writing access of Client ',
                              client_id, ' revoked')
